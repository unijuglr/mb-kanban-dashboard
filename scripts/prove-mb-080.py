...
class FakeNeo4jHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "RETURN 1 AS ok" in statement_text:
            ...
            title = first_parameters.get("title")
            rows = []
...
def parse_sample(sample_path: str, cache_path: str):
    ...
    link_titles = [sorted(entity.get("metadata", {}).get("links", [])) for entity in entities]
    return entities, titles, link_titles


def run_offline_proof(sample_path: str, cache_path: str) -> Dict[str, Any]:
    entities, titles, link_titles = parse_sample(sample_path, cache_path)
    ...
    constraint_payloads = [
        req
        for req in captured
        if any("CREATE CONSTRAINT entity_olid" in stmt.get("statement", "") for stmt in req["payload"].get("statements", []))
    ]
    merged_entities_payload = entities

    result = {
        "sample_path": sample_path,
        "primary_titles": titles,
        "primary_entities_parsed": len(entities),
        "primary_link_titles": link_titles,
    ...
    assert entity_count_rows[0]["entity_count"] == 7
    assert mentions_count_rows[0]["mentions_count"] == 7
    assert bool(luke_rows) is True
    assert bool(tatooine_rows) is True
    assert relation_rows[0]["rel_count"] >= 1
...
def try_live_probe(sample_path: str, cache_path: str, uri: str, user: str, password: str, database: str) -> Dict[str, Any]:
    entities, titles, _ = parse_sample(sample_path, cache_path)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Proof MB-080: 2-page sample corpus and repo-side executable ingest proof.")
    ...
    parser.add_argument("--output", default=None, help="Optional path to save the proof JSON output")
    parser.add_argument("--probe-live", action="store_true", help="Also attempt a real Neo4j probe; do not use this to claim Motherbrain proof")
    ...
    output: Dict[str, Any] = {
        "offline_contract_proof": run_offline_proof(args.sample, args.cache_path),
        "live_neo4j_probe": {"attempted": False, "reason": "not requested; live Neo4j proof remains downstream MB-087/MB-088 work"},
    }
