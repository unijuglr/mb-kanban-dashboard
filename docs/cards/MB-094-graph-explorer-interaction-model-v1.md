...
## Smallest Honest Acceptance Proof
1. Start the real dashboard dev server and open `/graph` in a browser.
2. Enter a term that produces at least one entity hit and at least one non-entity hit class (`relationship` or `path`).
3. Verify the UI visibly groups results by type instead of flattening everything into one generic list.
4. Single-click one grouped result or one rendered node.
5. Verify in the same interaction that:
   - the detail panel updates to the selected target, and
   - the graph visibly refocuses/highlights that target's local neighborhood.
6. Double-click that same node/result.
7. Verify navigation lands on a defined destination route or explicit placeholder route; no silent no-op.
8. Capture proof from the live route with both network evidence (`/graph`, and if used `/api/graph`) and visual evidence of grouped results + inspect/focus + double-click destination.

## Fake-Completion / QA Traps
- Search only filters existing node labels; that is not enough if "relationship" and "path" buckets are claimed.
- Clicking a list row that only changes side-panel text without changing graph focus is fake MB-094 completion.
- Highlighting the selected node without showing neighborhood/context is weaker than the stated contract.
- Double-click handlers that only exist in code or log to console do not count; navigation must be user-visible.
- Hardcoded/demo search buckets that are not derived from the real running explorer data contract do not count.
- Screenshot-only proof is insufficient; include executable steps or route/network checks against the running app.
