...
## 2026-04-04 reconciliation on current repo state
This card was restored onto a fresh branch from current `feat/mb-089-graph-explorer-mvp` state instead of relying on the older MB-078 branch alone.

Observed compatibility drift during the first rerun:
- `python3 scripts/qa_agilitas_pipeline.py` initially failed with `TypeError: __init__() got an unexpected keyword argument 'deterministic_fallback'`
- Current `services/agilitas-ai-core/extractor.py` no longer accepts the old `deterministic_fallback` init arg and no longer exposes a `provider=` parameter on `extract_dimensions(...)`

Reconciliation fix:
- Updated `scripts/qa_agilitas_pipeline.py` only, keeping scope tight
- The script now detects extractor API shape at runtime and preserves the honest deterministic fallback path when Ollama is unavailable

Fresh rerun on the reconciled branch:
```bash
python3 scripts/qa_agilitas_pipeline.py
python3 scripts/test_agilitas_redaction.py
python3 scripts/test_agilitas_ingestor.py
...
Observed results:
- proof pipeline: PASS, artifact rewritten with `run_id: 20260404T120534Z-mb-078-retail-proof`
- provider result: `deterministic-fallback`
- provider warning captured: `Ollama Error: HTTPConnectionPool(host='127.0.0.1', port=11434) ... [Errno 61] Connection refused`
- redaction test: PASS using `FallbackRedactor`
- ingestor normalization test: PASS
